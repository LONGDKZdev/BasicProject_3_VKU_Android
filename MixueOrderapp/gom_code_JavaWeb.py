...
# 2. Lọc danh sách thư mục rác, thư viện, thư mục tự build của Java Web
EXCLUDE_DIRS = {
    '.git', '.idea', '.vscode', 'nbproject',       # Thư mục của IDE / Git
    'target', 'build', 'out', 'bin',              # Thư mục build của Maven, Gradle, Java
    '.gradle', '.mvn',                            # Công cụ build
    'node_modules', 'bower_components',           # Thư viện Frontend (nếu có mix)
    'static', 'assets', 'images', 'dist'          # Bỏ qua tài nguyên tĩnh/ảnh nặng (nếu không cần thiết)
}

# 3. Chỉ quét các đuôi file có chứa logic/giao diện của Java Web
INCLUDE_EXTENSIONS = {
    # Backend Java
    '.java', '.groovy', 
    # Cấu hình dự án
    '.xml', '.properties', '.yml', '.yaml', 
    # Frontend / View (Spring Boot, JSP, Servlet)
    '.html', '.xhtml', '.jsp', '.jspf', '.tag', '.ftl', 
    # CSS & JavaScript xử lý logic giao diện
    '.css', '.js', '.ts', '.json'
}

# 4. Những file đặc biệt nên bỏ qua (bảo mật, file nhị phân hoặc cấu hình môi trường cục bộ)
EXCLUDE_FILES = {
    '.DS_Store', 'Thumbs.db',
    'mvnw', 'mvnw.cmd', 'gradlew', 'gradlew.bat', # Script chạy build hệ thống
    'system.properties',                           # Thư mục config môi trường chạy Production (Heroku...)
    # Bạn có thể cân nhắc bỏ qua application-secret.properties/yml nếu chứa mật khẩu DB thật
}

OUTPUT_PREFIX = 'gom_code_java_web_part_'
...
def generate_project_chunks():
    ...
    current_content.append(f"# TỔNG HỢP MÃ NGUỒN DỰ ÁN JAVA WEB (PHẦN {current_chunk_idx})\n")
                ...
                try:
                    with open(filepath, 'r', encoding='utf-8') as f:
                        ...
                        if current_char_count + block_len > MAX_CHARS_PER_FILE and current_char_count > 0:
                            ...
                            current_chunk_idx += 1
                            current_content = [f"# TỔNG HỢP MÃ NGUỒN DỰ ÁN JAVA WEB (PHẦN {current_chunk_idx})\n" + "=" * 50 + "\n"]
...
if __name__ == "__main__":
    print("⏳ Đang quét và chia nhỏ mã nguồn Java Web...")
